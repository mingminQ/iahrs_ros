...
namespace // anonymous namespace for internal linkage
{
    ...
    static const char *log_header[4] = {"[IAHRS Debug]: "   ,
                                        "[IAHRS Info]: "    ,
                                        "[IAHRS Warning]: " ,
                                        "[IAHRS Error]: "   };
    ...
    struct LogManager
    {
        ...
        iahrs::log::LogLevel log_level_;

        // Activated handler type
        iahrs::log::LogType log_type_;

        // Pointer to active handler
        iahrs::log::LogHandler *current_handler_;

        // Terminal log handler instance
        iahrs::log::LogHandlerTerminal *terminal_log_handler_;

        //  File handler instance, if enabled
        iahrs::log::LogHandlerFile *file_log_handler_;
        ...
        LogManager()
        {
            ...
            log_level_ = iahrs::log::LogLevel::INFO;

            // File log handler is not activated at default
            terminal_log_handler_ = new iahrs::log::LogHandlerTerminal();
            file_log_handler_     = nullptr;

            // Default log handler is terminal log handler
            log_type_        = iahrs::log::LogType::TERMINAL;
 ...
 */
void iahrs::log::LogHandlerTerminal::print_log(LogLevel log_level, const std::string &msg)
{
    // Linux system only
    // bool isTTY(isatty(fileno(stderr)) != 0);
    bool isTTY = true;

    if(iahrs::log::LogLevel::WARNING <= log_level)
    {
        if(isTTY)
        {
    ...
    else // log_level <= iahrs::log::LogLevel::INFO
    {
        if(isTTY)
        {
 ...
 */
iahrs::log::LogHandlerFile::LogHandlerFile(const char *file_name)
{
...
iahrs::log::LogHandlerFile::~LogHandlerFile()
{
    if(file_ != nullptr)
    {
        if(fclose(file_) != 0)
 ...
 * @param msg Formatted log message
 */
void iahrs::log::LogHandlerFile::print_log(LogLevel log_level, const std::string &msg)
{
    if(file_ != nullptr)
    {
        if(iahrs::log::LogLevel::WARNING <= log_level)
 ...
 */
void iahrs::log::set_log_level(LogLevel log_level)
{
    GET_LOG_MANAGER;
 ...
 */
iahrs::log::LogLevel iahrs::log::get_log_level()
{
    GET_LOG_MANAGER;
 ...
 * @param file_name Path to the log file
 */
void iahrs::log::activate_log_file(const std::string &file_name)
{
    GET_LOG_MANAGER;
...
void iahrs::log::deactivate_log_file()
{
    GET_LOG_MANAGER;
    if(log_manager->file_log_handler_ == nullptr)
    {
 ...
 */
void iahrs::log::set_log_type(LogType log_type)
{
    GET_LOG_MANAGER;
    if(log_type != log_manager->log_type_)
    {
        ...
        else // log_type == iahrs::log::TERMINAL_HANDLER
        {
 ...
 */
iahrs::log::LogType iahrs::log::get_log_type()
{
    GET_LOG_MANAGER;
 ...
 */
void iahrs::log::print_log(LogLevel log_level, const char *msg, ...)
{
    GET_LOG_MANAGER;
    if(log_manager->current_handler_ != nullptr && log_manager->log_level_ <= log_level)
    {
        ...
        log_manager->current_handler_->print_log(log_level, buffer);
    }
}